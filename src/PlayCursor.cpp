...
PlayCursor::PlayCursor(GridLocation * gridLocation)
{
	...
	m_sender.setup( HOST, SENDING_PORT );		
}
...
void PlayCursor:: sendOSCMessage(Block * block, string playMode, bool resetTempo, bool resetTimbre)
{
	...
	sentMsg = "id 2, source midi, " + m_chordBook->chord2notes(block->getBaseNote(), block->getChordType(), 4) 
	+ ", parameters " + playMode + " 2 140 " + (resetTempo?"1":"0") + " " + (resetTimbre?"-1":"0");
...
void PlayCursor::sendOSCMessage(int MIDINote, int velocity)
{
	...
	string sentMsg;
	sentMsg = "id 2, source midi, " "notes "+ ofToString(MIDINote, 0) + ", parameters notekey 4 " + ofToString(velocity, 0);
